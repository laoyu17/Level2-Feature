...
- `io/reader.py`：文件读取、过滤与可选 canonicalize 入口
- `io/adapter.py`：字段别名映射与标准 schema 归一化
...
- 当前实现为先读取并 collect 到 DataFrame，再执行链式特征计算；在不改变行为前提下减少中间拷贝
- 单次 scan/collect 属于 lazy 场景的优化目标，不作为当前实现前提
- 流式波动率使用 numba（可选）加速
