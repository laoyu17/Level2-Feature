...
def test_validate_required_columns_rejects_incomplete_depth_level() -> None:
    df = pl.DataFrame(
        {
            "ts": [1],
            "symbol": ["000001.SZ"],
    ...
    with pytest.raises(ValueError, match="Incomplete depth columns"):
        validate_required_columns(df)
